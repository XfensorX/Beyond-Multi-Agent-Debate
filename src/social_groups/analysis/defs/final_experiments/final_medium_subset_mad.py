...
def final_medium_subset_mad(combined_data: pl.DataFrame):
    ...
    yield check_unique_data_connector(final_medium_subset_mad)
    yield (
        check_correct_data_length(
            final_medium_subset_mad, 300 * (3 * 3 + 3 * 6 + 3 * 10 + 3 * 15)
        )
    )

    yield dg.Output(final_medium_subset_mad)


defs = dg.Definitions(assets=dg.with_source_code_references([final_medium_subset_mad]))
