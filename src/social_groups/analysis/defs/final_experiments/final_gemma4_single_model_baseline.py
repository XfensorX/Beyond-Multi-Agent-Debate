...
@dg.asset(
    ...
    check_specs=[
        AssetCheckSpec(
            ...
            asset="final_gemma4_single_model_baseline",
            blocking=True,
        ),
        AssetCheckSpec(
...
def final_gemma4_single_model_baseline(combined_data: pl.DataFrame):
    final_gemma4_single_model_baseline = (
        combined_data.filter(
            (pl.col("name") == "gemma4_single_model_baseline")
    ...
    yield check_unique_data_connector(final_gemma4_single_model_baseline)
    yield check_correct_data_length(final_gemma4_single_model_baseline, 198 * 3 * 5)

    yield dg.Output(
        final_gemma4_single_model_baseline.drop(
            [
...
defs = dg.Definitions(
    assets=dg.with_source_code_references([final_gemma4_single_model_baseline])
)
