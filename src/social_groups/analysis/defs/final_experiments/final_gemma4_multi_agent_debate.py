...
@dg.asset(
    ...
    check_specs=[
        AssetCheckSpec(
            ...
            asset="final_gemma4_multi_agent_debate",
            blocking=True,
        ),
        AssetCheckSpec(
...
def final_gemma4_multi_agent_debate(combined_data: pl.DataFrame):
    final_gemma4_multi_agent_debate = (
        combined_data.filter(
            (
                pl.col("name").is_in(
                    {
                        "gemma4_multi_agent_debate",
                        "gemma4_multi_agent_debate_only_H_thinking",
                        "gemma4_multi_agent_debate_2nd_try",
                    }
                )
            )
        ...
        .with_columns(
            ...
            .replace(
                {
                    "gemma4_multi_agent_debate": "all",
                    "gemma4_multi_agent_debate_only_H_thinking": "onlyH",
                    "gemma4_multi_agent_debate_2nd_try": "all2ndTry",
                }
            )
    ...
    yield check_unique_data_connector(final_gemma4_multi_agent_debate)
    yield check_correct_data_length(
        final_gemma4_multi_agent_debate, 2 * 198 * 4 * 5 + 198 * 5 * 7
    )

    yield dg.Output(
        final_gemma4_multi_agent_debate.drop(
            [
                "experiment_id",
                "name",
...
defs = dg.Definitions(
    assets=dg.with_source_code_references([final_gemma4_multi_agent_debate])
)
