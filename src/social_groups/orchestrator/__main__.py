...
from social_groups.orchestrator.services.base import SlurmServiceName, load_config
from social_groups.orchestrator.services.experiment import (
    ExperimentConfiguration,
    ExperimentStartingInformation,
)
from social_groups.orchestrator.services.phoenix import PhoenixConfiguration
from social_groups.orchestrator.services.tgi import (
    TgiConfiguration,
...
async def run_service_on_slurm(
    ...
    llm: Optional[list[str]] = typer.Option(
        ...
        "--llm",
        "-l",
        help="If starting TGI, declaring which LLMs to start. Can be used several times to start several services.",
    ),
    ...
    if isinstance(service_config, TgiConfiguration):
        if not llm:
            raise typer.BadParameter(
        ...
        for llm_name in llm:
            ...
            service_config.set_used_model(llm_name)
        ...
        tgi_jobs: list[SlurmJobInfo] = list(
            filter(
                lambda j: TgiConfiguration.job_name_is_matching_this_service(
                    j.job_name
                ),
                current_jobs,
            )
        ...
        print(
            f"Detected following TGI Backends running: {[j.job_name for j in tgi_jobs]}"
        )
        ...
        running_llm_models = {
            model_id: (info, job)
            for model_id, info in tgi_config.llm_models.items()
            if (
                job := next(
                    (
                        j
                        for j in tgi_jobs
                        if model_id_to_job_name_appendix(model_id) in j.job_name
                    ),
                    None,
                )
            )
        ...
        service_config.add_starting_info(
            ExperimentStartingInformation(
                ...
                model_backends=[
                    ...
                    for llm, (info, job) in running_llm_models.items()
                ],
            )
        )
...
def stop_service(where: ExecutionLocation, service: SlurmServiceName):
    service_config = load_config(service, where)
    exec_config = load_execution_config(where)
    cmd = f"scancel -n {service_config.used_job_name}"
    # TODO: add custom behaviour of used_job_name here, maybe just use a multi-select or so
